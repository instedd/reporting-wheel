class Wheel < ActiveRecord::Base
  ...
  has_many :wheel_rows, {:dependent => :destroy}, proc { order "wheel_rows.index ASC" }
  belongs_to :user
  belongs_to :pool
  ...
  def save_success_voice
    ...
    return if @success_voice_file.blank?

    FileUtils.mkdir_p(absolute(audio_directory))

    File.open(absolute_path, "w") { |f| f.write(@success_voice_file.read); }
  end
  ...
  def save_cover_image
    ...
    return if @cover_image_file.blank?

    FileUtils.mkdir_p(absolute(images_directory))

    File.open(absolute_path, "w+b") { |f| f.write(@cover_image_file.read); }
  end
